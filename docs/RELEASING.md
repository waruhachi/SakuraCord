...
Release tags publish the exact reviewed commit they reference. Regular releases
do not require a parallel nightly branch. The generated `nightly-feed` branch
only hosts the signed Sparkle appcast for optional beta releases and must not be
edited by hand.
...
For an optional nightly beta, tag and push the reviewed commit directly:
